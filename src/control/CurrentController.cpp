...
CurrentController::CurrentController(const CurrentControlParams& p)
    : p_(p), pi_d_(0.0, 0.0, PIController::kDefaultLimits), pi_q_(0.0, 0.0, PIController::kDefaultLimits) {}
...
Vec2 CurrentController::limit_vdq_spwm(const Vec2& v_dq) const {
    ...
    if (mag <= vmax || mag <= 1e-12) {
        return v_dq;
    }
    return v_dq * (vmax / mag);
}

Vec2 CurrentController::step(const Vec2& i_dq_ref, const Vec2& i_dq_meas, double omega_e, double dt) {
    ...
    return limit_vdq_spwm({v_d_pi + v_d_ff, v_q_pi + v_q_ff});
}
