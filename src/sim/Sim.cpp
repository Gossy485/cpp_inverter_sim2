...
Sim::Sim(const SimParams& sp, const PMSMParams& mp)
    : sp_(sp),
      ...
      speed_controller_(SpeedControlParams{-50.0, 50.0}),
      inverter_(sp.Vdc),
      spwm_(sp.Vdc),
      logger_(sp.csv_path) {
    ...
    const double w_sc = 2.0 * kPi * 40.0;
...
double Sim::speed_ref(double t) const {
    if (sp_.omega_ref_ramp_time <= 1e-12 || t >= sp_.omega_ref_ramp_time) {
        return sp_.omega_ref_step;
    }
    return sp_.omega_ref_step * (t / sp_.omega_ref_ramp_time);
}
